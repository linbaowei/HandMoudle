...
pcl::PointCloud< PointType > Geomentry::generateSphere(pcl::PointCloud< PointType > BarycenterC, float radius, bool flag)
// if flag == true, sphere for index and thumb finger (small sphere)
// if flag == false, sphere for hand end points (big sphere)
{
  ...
  for(int i = 0; i <= 2*step; ++ i)
  {
    ...
    for(int j = 0; j < spherepoints.size(); ++ j)
    {
      ...
      yAxis[1] = 1;
      yAxis[2] = 0;
      float tmpangle = VectorAngle(newpoint, yAxis);
      if(flag)
      {
	if(tmpangle > 3.14/4)
	{
	  ...
	  point.y = newpoint[1];
	  point.z = newpoint[2];
	  if((j == 0 || j == spherepoints.size()-1) && i > 0)
	    continue;
...
string Geomentry::numToString(int i)
{
